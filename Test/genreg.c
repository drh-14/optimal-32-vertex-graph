...
double global_min_aspl = DBL_MAX;

/* Structure for storing a CSV entry for a best graph */
typedef struct BestGraph {
   char *csv_entry;
   struct BestGraph *next;
} BestGraph;
...
static SCHAR springen,i1,j_one,f0,f1,girth_exact;
...
void add_best_graph(const char *entry) {
   BestGraph *new_node = (BestGraph *) malloc(sizeof(BestGraph));
   if (!new_node) {
       fprintf(stderr, "Memory allocation error in add_best_graph\n");
       exit(1);
   }
   new_node->csv_entry = strdup(entry);
   if (!new_node->csv_entry) {
       fprintf(stderr, "Memory allocation error in add_best_graph\n");
       exit(1);
   }
   new_node->next = best_graphs;
   best_graphs = new_node;
}
...
void store_best_graph(void) {
   double aspl = compute_aspl();  // Compute the ASPL for the current graph
   char buffer[BUFFER_SIZE];
   int pos = 0, i, j;

   /* Build the CSV entry */
   pos += snprintf(buffer + pos, BUFFER_SIZE - pos, "Graph %lu\n", anz);
   pos += snprintf(buffer + pos, BUFFER_SIZE - pos, "minASPL: %.6f\n", aspl);
   pos += snprintf(buffer + pos, BUFFER_SIZE - pos, "Adjacency list:\n");
   for (i = 1; i <= n; i++) {
       pos += snprintf(buffer + pos, BUFFER_SIZE - pos, "%d:", i);
       for (j = 1; j <= grad[i]; j++) {
           pos += snprintf(buffer + pos, BUFFER_SIZE - pos, " %d", l[i][j]);
       }
       pos += snprintf(buffer + pos, BUFFER_SIZE - pos, "\n");
   }
   pos += snprintf(buffer + pos, BUFFER_SIZE - pos, "\n");

   /* Use environment variable GLOBAL_MIN if provided; otherwise, use global_min_aspl */
   double threshold = global_min_aspl;
   char *env = getenv("GLOBAL_MIN");
   if (env)
       threshold = atof(env);

   /* Compare with the threshold */
   if (aspl < threshold - TOLERANCE) {
       global_min_aspl = aspl;
       flush_best_graphs();
       add_best_graph(buffer);
   } else if (fabs(aspl - threshold) < TOLERANCE) {
       add_best_graph(buffer);
   }
...
void komprtofile() {
   ...
   FILE *csvFile;
   int i, j;
   double aspl = compute_aspl();

   /* Create the folder name from the jobs parameter, e.g., "5000partitions" */
   sprintf(foldername, "%dpartitions", jobs);
   ...
   /* Create filename inside the folder using jobnr (1-based) and jobs */
   sprintf(filename, "%s/%d_%d.csv", foldername, jobnr + 1, jobs);
   ...
   /* Write header and ASPL value */
   fprintf(csvFile, "Graph %lu\n", anz);
   fprintf(csvFile, "minASPL: %.6f\n", aspl);
   fprintf(csvFile, "Adjacency list:\n");

   /* Write the adjacency list */
   for (i = 1; i <= n; i++) {
       fprintf(csvFile, "%d:", i);
       for (j = 1; j <= grad[i]; j++) {
           fprintf(csvFile, " %d", l[i][j]);
       }
       fprintf(csvFile, "\n");
   }
   fprintf(csvFile, "\n");  // Separate graphs with an empty line
...
SCHAR mx,my,vm,tw,lblock;
{
 ...
 if(mx == n && grad[n] == k)  /* Graph is complete/regular */
{
    if(maxstartneu(tw))
    {
        anz++;
...
void output_best_graphs_to_csv(void) {
   ...
   /* Create folder name based on jobs value, e.g. "5000partitions" */
   sprintf(foldername, "%dpartitions", jobs);
   ...
   BestGraph *cur = best_graphs;
   while (cur) {
       fprintf(csvFile, "%s", cur->csv_entry);
       cur = cur->next;
   }
   fclose(csvFile);
}